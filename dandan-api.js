...
export async function extractTitleSeasonEpisode(cleanFileName) {
  ...
  const normalizeMatchedTitle = (rawTitle) => {
    if (!rawTitle) return '';

    return rawTitle
      .replace(/[._]/g, ' ')
      .replace(/\s*[\(\[（【]\s*(?:普通话|国语|中文配音|中配|中文|粤配|粤语|台配|台语|港配|港语|日配|日语|原版|原声|双语|字幕|助听)(?:版)?\s*[\)\]）】]\s*/gi, ' ')
      .replace(/\s*(?:普通话|国语|中文配音|中配|中文|粤配|粤语|台配|台语|港配|港语|日配|日语|原版|原声|双语|字幕|助听)(?:版)?\s*/gi, ' ')
      .replace(/\s+/g, ' ')
      .trim();
  };
  ...
  if (match) {
    ...
    // 优先解析更常见的中文/EP集数格式，例如：
    // 名侦探柯南（中配） 第0004集 / foo EP12 / foo E03
    const episodePatterns = [
      /^(.*?)[.\s_-]*(?:第\s*0*(\d+)\s*[集话話]|第\s*([一二三四五六七八九十百零〇两壹贰叁肆伍陆柒捌玖拾]+)\s*[集话話])(?:\s|$)/i,
      /^(.*?)[.\s_-]*[Ee][Pp]?\s*0*(\d+)(?:\s|$)/i,
      /^(.*?)[.\s_-]*0*(\d+)\s*[集话話](?:\s|$)/i
    ];
    ...
    for (const pattern of episodePatterns) {
      const episodeMatch = cleanFileName.match(pattern);
      if (!episodeMatch) continue;

      explicitTitle = episodeMatch[1] || '';
      explicitEpisode = episodeMatch[2]
        ? parseInt(episodeMatch[2], 10)
        : episodeMatch[3]
          ? convertChineseNumber(episodeMatch[3])
          : null;

      if (explicitTitle && explicitEpisode) break;
    }

    // 没有 S##E## 格式时，尝试提取第一个片段作为标题
    ...
    title = explicitTitle
      ? normalizeMatchedTitle(explicitTitle)
      : titleMatch
        ? normalizeMatchedTitle(titleMatch[1])
        : normalizeMatchedTitle(cleanFileName);
    season = explicitEpisode ? 1 : null;
